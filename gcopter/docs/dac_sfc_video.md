...
| 8–12 s | 高亮所选路段 |
| 12–18 s | Easy / Middle / Hard 双向特征向量；绿色 probe 继续显示 |
...
probe 从 4 秒开始一直显示到结束，保持论文原先的绿色和原先出现的时刻。
它可能是一条高高拱起的平滑曲线，与表示局部微扰方向的绿色 Easy 箭头含义不同，
录制时请按论文说明区分。预算及盒子生长阶段继续显示绿色 Easy 与紫色 Hard 的
真实 `extra` 数字，例如 `Easy=3.00 m`、`Hard=0.75 m`。三条标注引线均只连到
双向箭头在标签下方的端点，引线的末端也停在名称和数值两行文字之下。
